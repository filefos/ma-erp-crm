      ...
      {sections.map((sec, si) => {
        ...
        return (
          <div key={si} className="mb-0">
            ...
            >
              <span className="text-white font-bold text-[11px] shrink-0">{headerNum}.</span>
              <input
                className="flex-1 text-white font-bold text-[11px] uppercase bg-transparent border-0 outline-none placeholder-white/40 tracking-wide"
                value={titleBody}
            ...
            <table className="w-full border-collapse">
              <tbody>
                {sec.items.length === 0 ? (
                  <tr>
                    <td className="border border-gray-200 px-3 py-1 text-gray-400 italic text-[11px]">
                  ...
                  sec.items.map((item, ii) => {
                    ...
                    return (
                      ...
                      >
                        <td className="border border-gray-200 px-2 py-1 text-center font-semibold text-gray-500 align-top shrink-0" style={{ width: 28 }}>
                          {ii + 1}.
                        </td>
                        <td className={`border border-gray-200 px-2 py-0.5 align-top ${isCheque ? "bg-blue-50" : ""}`}>
                          <textarea
                            className={`w-full resize-none border-0 outline-none bg-transparent leading-snug py-0.5 text-[11px] ${isCheque ? "font-semibold text-[#0f2d5a]" : ""}`}
                            value={body}
                            rows={Math.max(1, Math.ceil((body.length || 1) / 100))}
                            placeholder="Enter item text..."
                            onChange={e => updateItem(si, ii, `${ii + 1}. ${e.target.value}`)}
                          />
                        </td>
                        <td className="border border-gray-200 p-1 text-center align-top" style={{ width: 28 }}>
                          <button
                            type="button"
