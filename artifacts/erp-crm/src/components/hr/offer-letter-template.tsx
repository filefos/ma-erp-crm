...
const OL_COMPANIES: Record<number, { name: string; address: string; phone: string; email: string; website: string; contact: string }> = {
  1: {
...
export const OfferLetterTemplate = forwardRef<HTMLDivElement, { doc: OfferLetterDoc }>(({ doc }, ref) => {
  ...
  return (
    ...
    >
      ...
      {/* ── LETTERHEAD (identical to UndertakingLetterTemplate) ── */}
      ...
      {/* Body wrapper — flex-1 pushes the signature + footer to the bottom of the page. */}
