...
import {
  Clock, CheckCircle2, X, FileText, FileCheck, Truck, ReceiptText, ShoppingCart, ClipboardList
} from "lucide-react";
...
export function DelegatedTaskPopup() {
  ...
  useEffect(() => {
    if (!activeTask) { setDone(false); return; }
  ...
  const handleOpenForm = () => {
    if (activeTask.leadId && activeTask.taskType === "quotation") {
      navigate(`/sales/quotations/new?leadId=${activeTask.leadId}`);
    } else {
      navigate(meta.href);
    }
  };
      ...
      >
        ...
        <div className="px-5 pb-4 text-center text-[12px]" style={{ color: "#a19f9d" }}>
          {expired
            ...
            : "This popup is the only thing you can do right now. Complete the task or wait for time to expire."}
        </div>

        {/* Close (only when expired or done) */}
        {(expired) && (
          <button
            onClick={dismissTask}
