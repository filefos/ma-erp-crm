...
import {
  ...
  ChevronLeft, Search, RefreshCw, MoreVertical, Archive,
  Eye, EyeOff, Pencil
} from "lucide-react";
import {
...
export function EmailPanel() {
  ...
  const { data: emails = [], isLoading, refetch } = useQuery<Email[]>({
    queryKey: emailsKey,
    queryFn: () => {
      const params = new URLSearchParams({ folder: folder === "starred" ? "" : folder });
  ...
  const displayEmails = folder === "starred"
  ...
  const logReceivedMutation = useMutation({
    mutationFn: (data: any) => apiFetch("/emails", { method: "POST", body: JSON.stringify({ ...data, folder: "inbox" }) }),
    onSuccess: () => {
      toast({ title: "Email logged to inbox." });
      qc.invalidateQueries({ queryKey: ["emails"] });
    },
  });
  ...
  const handleSend = () => {
    ...
    sendMutation.mutate({ ...compose, action: "send" });
  };
      ...
      <div className="w-52 flex-shrink-0 border-r border-gray-200 bg-gray-50 flex flex-col">
        <div className="p-3">
          <Button
            ...
            onClick={() => { setComposing(true); setSelectedId(null); setCompose({ toAddress: "", toName: "", ccAddress: "", subject: "", body: "" }); }}
          >
        ...
        <div className="p-3 border-t border-gray-200">
          <Button
            ...
            onClick={() => {
              const from = prompt("Sender email address:");
              if (!from) return;
              const subj = prompt("Subject:");
              if (!subj) return;
              const body = prompt("Email body:");
              logReceivedMutation.mutate({ fromAddress: from, toAddress: user?.email ?? "info@primemaxprefab.com", subject: subj, body: body ?? "" });
            }}
          >
            <Plus className="w-3 h-3 mr-1" /> Log Received
          </Button>
        </div>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-1 mb-0.5">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  {!email.isRead && email.folder === "inbox" && <span className="w-2 h-2 rounded-full bg-[#1e6ab0] flex-shrink-0" />}
        ...
        {composing ? (
          ...
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => saveDraftMutation.mutate({ ...compose, folder: "draft" })}>
                  Save Draft
                </Button>
                <Button
                  size="sm"
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              ...
              <div className="text-xs text-gray-400 border-t pt-2">
                {process.env.NODE_ENV !== undefined && (
                  <span>
                    Email will be sent via SMTP if configured (SMTP_HOST, SMTP_USER, SMTP_PASS). Otherwise saved to Sent folder only.
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="px-5 py-3 border-b border-gray-100 bg-white">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-0.5">
                  ...
                  <div className="text-xs text-gray-500 mt-1.5 pl-11">
                    <span className="font-medium">To:</span> {selectedEmail.toAddress}
                    {selectedEmail.ccAddress && <><span className="font-medium ml-3">CC:</span> {selectedEmail.ccAddress}</>}
                  </div>
                </div>
                <div className="text-xs text-gray-400 text-right flex-shrink-0">
                  ...
                  {selectedEmail.folder === "inbox" && !selectedEmail.isRead && (
                    <div className="mt-1"><Badge className="text-[10px] bg-blue-100 text-blue-700">Unread</Badge></div>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-3">
            ...
            <Button
              ...
              onClick={() => { setComposing(true); setCompose({ toAddress: "", toName: "", ccAddress: "", subject: "", body: "" }); }}
            >
