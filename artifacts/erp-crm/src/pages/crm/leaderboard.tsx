...
import {
  useListLeads, useListDeals, useListActivities, useListUsers,
} from "@workspace/api-client-react";
import { useActiveCompany } from "@/hooks/useActiveCompany";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  Trophy, TrendingUp, Users, Award, Target, ArrowRight, Crown, Star, Medal,
  BarChart3,
} from "lucide-react";
import {
...
export function SalesLeaderboard() {
  ...
  const rows: Row[] = useMemo(() => {
    return users.map((u: any) => {
      ...
      const myActs  = activities.filter((a: any) => a.createdById === u.id);
      const won = myDeals.filter(d => d.stage === "won");
      const active = myDeals.filter(d => !["won", "lost"].includes(d.stage));
      const wonValue = won.reduce((s, d) => s + Number(d.value ?? 0), 0);
      const pipelineValue = active.reduce((s, d) => s + Number(d.value ?? 0), 0);
      const conversion = myLeads.length > 0 ? Math.round((myLeads.filter(l => l.status === "won").length / myLeads.length) * 100) : 0;
      // Composite score: weight won value heaviest, then deals, then activities.
      // Conversion % only contributes once a rep has at least 5 leads, so a rep with
      // 1 lead won (100% conversion) doesn't outrank a high-volume performer.
      const conversionWeight = myLeads.length >= 5 ? conversion : 0;
      const score = Math.round(wonValue / 1000) + won.length * 50 + active.length * 10 + myActs.length * 2 + conversionWeight;
      return {
        id: u.id, name: u.name, role: u.role,
        leads: myLeads.length, hotLeads: myLeads.filter(l => l.leadScore === "hot").length,
        activeDeals: active.length, wonDeals: won.length,
        wonValue, pipelineValue, activities: myActs.length, conversion, score,
      };
    }).sort((a, b) => b.score - a.score);
  }, [users, leads, deals, activities]);
      ...
      {top && top.score > 0 && (
        <div className="bg-gradient-to-r from-[#0f2d5a] to-[#1e6ab0] rounded-xl p-5 text-white flex items-center gap-4 flex-wrap">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <Stat label="Won deals"     value={top.wonDeals} />
      ...
      {chartData.length > 0 && (
        <div className="bg-card border rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold">Won vs Open Pipeline (top 8)</h3>
          </div>
                ...
                <div key={r.id} className="p-3 flex items-center gap-3 hover:bg-muted/30" data-testid={`leaderboard-row-${r.id}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-0.5">
                      {r.leads} leads · {r.hotLeads} hot · {r.activities} activities
                    </div>
                  </div>
                  <div className="hidden md:grid grid-cols-4 gap-3 text-xs text-right">
                    <Stat label="Active" value={r.activeDeals} small />
                    <Stat label="Won" value={r.wonDeals} small />
