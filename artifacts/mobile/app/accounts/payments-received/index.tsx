...
import {
  ...
  useDeletePaymentReceived, useListBankAccounts, useListPaymentsReceived, useUpdatePaymentReceived,
...
export default function PaymentsReceivedList() {
  const c = useColors();
  const router = useRouter();
  const qc = useQueryClient();
  const { activeCompanyId } = useApp();
  const q = useListPaymentsReceived({ companyId: activeCompanyId ?? undefined });
  const data = useMemo(
    () => (q.data ?? []).filter(p => activeCompanyId == null || p.companyId === activeCompanyId),
    [q.data, activeCompanyId],
  );
  const del = useDeletePaymentReceived({
    mutation: {
      onSuccess: () => qc.invalidateQueries({ queryKey: getListPaymentsReceivedQueryKey() }),
  ...
  return (
    <View style={[styles.flex, { backgroundColor: c.background }]}>
      <AppHeader title="Payments received" subtitle={`${data.length} payment${data.length === 1 ? "" : "s"}`} />
      <ScrollView
        contentContainerStyle={styles.content}
        refreshControl={<RefreshControl refreshing={q.isRefetching} onRefresh={() => q.refetch()} tintColor={c.primary} />}
      >
        <BrandButton label="New payment" icon="plus" onPress={() => router.push("/accounts/payments-received/new")} />

        {q.isLoading ? <LoadingBlock /> : null}
        {q.error ? <ErrorBlock message={(q.error as Error).message ?? ""} onRetry={() => q.refetch()} /> : null}
        {!q.isLoading && data.length === 0 ? <EmptyState icon="download" title="No payments yet" hint="Record receipts from customers." /> : null}
        ...
        {data.map(p => (
          <Card key={p.id}>
            ...
            <Text style={[styles.body, { color: c.foreground }]} numberOfLines={1}>{p.customerName}</Text>
            {p.invoiceRef ? <Text style={[styles.meta, { color: c.mutedForeground }]}>Invoice: {p.invoiceRef}</Text> : null}
            <View style={styles.row}>
              <Text style={[styles.amount, { color: c.success }]}>{fmtAed(p.amount)}</Text>
...
function EditPaymentModal({ visible, payment, onClose }: { visible: boolean; payment: PaymentReceived | null; onClose: () => void }) {
  const c = useColors();
  const qc = useQueryClient();
  const { activeCompanyId } = useApp();
  const banks = useListBankAccounts({ companyId: activeCompanyId ?? undefined });
  const update = useUpdatePaymentReceived({
    mutation: {
      onSuccess: () => { qc.invalidateQueries({ queryKey: getListPaymentsReceivedQueryKey() }); onClose(); },
      onError: (e: unknown) => Alert.alert("Update failed", (e as Error).message ?? ""),
    },
  });
  const [form, setForm] = useState({
    customerName: "", invoiceRef: "", paymentDate: "", amount: 0, paymentMethod: "bank_transfer",
    bankAccountId: "", referenceNumber: "", notes: "", status: "received",
  });
  React.useEffect(() => {
    if (payment) setForm({
      customerName: payment.customerName, invoiceRef: payment.invoiceRef ?? "",
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} presentationStyle="formSheet">
      <View style={[styles.flex, { backgroundColor: c.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <BrandInput label="Customer name" value={form.customerName} onChangeText={v => upd({ customerName: v })} />
          ...
          <View style={styles.row}>
            ...
            <BrandButton label="Save" loading={update.isPending} onPress={() => update.mutate({
              ...
              data: {
                ...
                customerName: form.customerName, invoiceRef: form.invoiceRef,
